...
export const execute = async (client, message, args, isWs = false) => {
  ...
  if (greetingWords?.some(word => messageContents.search(new RegExp(`\\b${word?.greeting}\\b`, "gmi")) > -1)) {
    ...
    if (isWs) wsReply(client, message, `${greeting[0]?.toUpperCase() + greeting?.substring(1)} ${tagUser}, サメです。 <a:guraWave:840734876964749342>`);
    else message.channel.send(`${greeting[0]?.toUpperCase() + greeting?.substring(1)} ${tagUser}, サメです。 <a:guraWave:840734876964749342>`);
